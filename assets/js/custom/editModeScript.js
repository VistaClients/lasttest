...
/* ---------------- SAVE HTML WITH BACKEND PATH ---------------- */
function prepareHTMLBeforeSave(wrapper) {
    const clientName = getCookie('clientName');
    const clientProjectName = getCookie('clientProjectName');
    ...
    wrapper.find('img').each(function () {
        ...
        if (originalSrc) {
            $(this).attr('src', `/static/${clientName}/${clientProjectName}/${originalSrc}`);
        }
    });
...
AIBridge.onMessage(data => {
    ...
    if (data.type === "MODEL_LOADING") {
        modelReady = false;
        $("#contentTextArea").val("Loading AI model... Please wait ⏳");
    }

    if (data.type === "MODEL_READY") {
        modelReady = true;
        $("#contentTextArea").val("AI model loaded ✔️");
...
function uploadeditedproject() {
  ...
  //  SHOW LOADER
    ...
    $.ajax({
        url: `/vistaaar/uploadeditedproject/${projectId}/`,
        type: "POST",
        data: {
